...
export function DossierRow({ label, value }: { label: string; value: string }) {
  return (
    <View className="flex-row items-baseline justify-between gap-3 py-1">
      <Text
        style={{ fontFamily: Fonts.mono, letterSpacing: 1 }}
        className="text-xs font-bold uppercase text-ink dark:text-paper-dark"
      >
        {label}
      </Text>
      <View className="mx-1 flex-1 border-b border-dashed border-[#D8D2C6] dark:border-[#39342E]" />
      <Text
        style={{ fontFamily: Fonts.mono }}
        className="shrink text-right text-xs text-ink dark:text-paper-dark"
      >
        {value}
      </Text>
    </View>
