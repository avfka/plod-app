  ...
  return (
    <Link href={{ pathname: '/event/[id]', params: { id: event.id } }} asChild>
      <Pressable
        ...
        className="overflow-hidden rounded-[4px] border border-ink bg-paper active:opacity-90 dark:border-paper-dark dark:bg-night-element"
      >
        <View className="flex-row items-center justify-between border-b border-ink bg-ink px-3 py-1.5 dark:border-paper-dark dark:bg-paper-dark">
          <Text
            style={{ fontFamily: Fonts.mono, letterSpacing: 1 }}
            className="text-[9px] font-bold uppercase text-paper dark:text-night"
          >
            Event / {event.id.slice(0, 6)}
          </Text>
          <Text
            style={{ fontFamily: Fonts.mono }}
            className="text-[9px] uppercase text-paper dark:text-night"
          >
            REC ●
          </Text>
        </View>
        <View className="flex-row">
          <View className="h-32 w-28 border-r border-ink dark:border-paper-dark">
            {event.photo_url ? (
          ...
          <View className="flex-1 gap-1.5 p-3">
            <View className="flex-row flex-wrap gap-1">
              ...
              {multiDay ? (
                <Tag label={`${event.event_sessions.length} дня`} color="#141210" />
              ) : null}
            </View>
        ...
        {multiDay ? <View className="h-[3px] bg-thread" /> : null}
      </Pressable>
    </Link>
  );
}
